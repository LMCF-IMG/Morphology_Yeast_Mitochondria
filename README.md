...
-  **The third, straightforward macro**, "MITO_IMAGE_BASED_SHAPES_INTENSITIES.ijm", operates by opening a segmented image in Fiji (containing the "_SEG_" string according to the convention specified below). The corresponding fluorescence image (with the "_pro_" string) must be in the same directory. The macro also opens it and performs an analysis of the intensities of individual mitochondria and their morphological characteristics. The results, saved to the disk in the same directory, include an image with numbered outlines of mitochondria and a CSV file containing their parameters. In the image, the mitochondria's numbers correspond to their indices in the table, listing area, average, minimum and maximum intensity, perimeter, length of the major and minor axes of the circumscribed ellipse, their angle, and [morphological characteristics](https://imagej.net/ij/docs/menus/analyze.html#set) – circularity, aspect ratio, roundness, solidity. In this case, the analysis is conducted collectively on the entire image and is not referenced to individual yeast cells.
...
**Branching Factor** = the SUM of all branch points of skeletons in the image / the SUM of all end points of skeletons in the image.
